...
This project contains maven project in the `protego-rules-signatures` subfolder.
This project is used to sign the policy rule classes.
Policy rules are loaded from `allow-nothing-policy-rules.yml` file that must be located in project's root folder.
...
The signature is made from hashing the class bytecode (SHA-1), from the bytecode is extracted the 8 bytes header (CAFE BABE + 4 bytes containing the class version).

* The first step is to generate the public and private keys (RSA-512) in `temp` subfolder (`key.pem` - private key, `publicKey.pub` - public key, `key.pkcs8` - public+private key pair) by running the `generate-certificate.sh` (or `.bat`) script.
*  Create a file named `allow-nothing-policy-rules.yml` in the project's root (the project that use the protego framework).
The file must have the key `publicKey` in the root, another key named `policyRuleCofigurations` which is a list of policy rule classes toghether with theirs signatures (`className` amd `signature`).
...
After that the file should look something like that:
[source,yaml]
----
publicKey: >-
